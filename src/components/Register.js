  ...
  return (
    <div className="authorization">
      <form className="popup__form" onSubmit={handleSubmit}>
        ...
        placeholder="E-mail" required onChange={handleEmail} value={email}/>
        <span className="popup__error"></span>
        <input type="password" className="popup__input popup__input_type_authorization" name="password" minLength="2" maxLength="25" 
        placeholder="Пароль" required onChange={handlePassword} value={password}/>
        <span className="popup__error"></span>
        <button className="popup__save popup__save_authorization">Зарегистрироваться</button>
          <Link to="/signin" className="authorization__login">Уже зарегистрированы? Войти</Link>
      </form>
    </div>
  )
}
